...
if __name__=='__main__':
   ...
   # move_Joint(1.57,-2.27,1.93,-1.19,-1.57,0) #home pose
    ...
    while not rospy.is_shutdown():
        ...
        goal_x = input("X: (1.5~2)")
        goal_y = input("Y: (-0.2~0.2)")
        goal_z = input("Z: (0.5~0.7)")

        ar_pose = Pose()
        ar_pose.position.x = sqrt(ur_reco_reach**2-(goal_z-0.39)**2)+0.25-0.05
        ar_pose.position.y = goal_y
        ar_pose.position.z = goal_z
        ...
        dist_x = 0
        prev_x = x
        dist_to_go = goal_x - ar_pose.position.x-0.25 #0.25: husky base와 ur3 base사이 거리
        #dist_ar = 0
        ...
        while  dist_to_go-dist_x > 0.05 : 
            ...
            desired_speed = 0.9/(np.exp(-12*last_dist+6)+1)+0.1
