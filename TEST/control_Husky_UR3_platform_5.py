...
if __name__=='__main__':
   ...
   # move_Joint(1.57,-2.27,1.93,-1.19,-1.57,0) #home pose
    ...
    while not rospy.is_shutdown():
        ...
        dist_x = 0
        prev_x = x
        dist_to_go = ar_x -ar_pose.position.x-0.25 #0.25: husky base와 ur3 base사이 거리
        while  dist_to_go > dist_x : 

            curr_x = x
            dist_x = curr_x-prev_x
            desired_speed = 1/(np.exp(-12*ar_x+6)+1)
        ...
        raw_input()
        #move_Joint(pi*90/180, 0 , -pi*90/180, pi*90/180, pi*90/180, 0)
        dist_x = 0
        prev_x = x
        while abs(prev_ar_z - ar_z)<0.02 :

            speed.linear.x = 0.1
            speed.angular.z = 0.0
            pub.publish(speed)
            print 'Searching for next target[AR_marker]...'
